...
#define OPEN_FILE(fp, filename, mode)   do { if ((fopen_s(&fp, filename, mode)) != 0) { fp = NULL; } } while (0)
...
#define OPEN_FILE(fp, filename, mode)   do { fp = fopen(filename, mode); } while (0)
...
// Regfile aliases
typedef enum {
    ZERO, RA, SP, GP, TP, T0, T1, T2, S0, FP = S0, S1, A0, A1, A2, A3, A4, A5, A6, A7, S2, S3, S4, S5, S6, S7,
        S8, S9, S10, S11, T3, T4, T5, T6, REGISTER_COUNT
} regfileAliases;
...
#define LOG_WARNING(msg)             printf("[rISA] [" FILENAME ":" LINE "] [WARNING]: " msg "\n")
#define LOG_ERROR(msg)               printf("[rISA] [" FILENAME ":" LINE "] [ERROR]: "   msg "\n")
#define LOG_INFO_PRINTF(fmt, ...)    printf("[rISA] [" FILENAME ":" LINE "] [INFO]: "    fmt "\n", ##__VA_ARGS__)
#define LOG_WARNING_PRINTF(fmt, ...) printf("[rISA] [" FILENAME ":" LINE "] [WARNING]: " fmt "\n", ##__VA_ARGS__)
#define LOG_ERROR_PRINTF(fmt, ...)   printf("[rISA] [" FILENAME ":" LINE "] [ERROR]: "   fmt "\n", ##__VA_ARGS__)

// Tracing macro with Register type syntax
#define TRACE_R(cpu, name) do { if (cpu->opts.o_tracePrintEnable) {          \
    printf("[rISA]:[TRACE]:[ %12d cycles ]:  %8x:   %08x   %s %s, %s, %s\n", \
        cpu->cycleCounter,                                                   \
        cpu->pc,                                                             \
...
#define TRACE_I(cpu, name) do { if (cpu->opts.o_tracePrintEnable) {          \
    printf("[rISA]:[TRACE]:[ %12d cycles ]:  %8x:   %08x   %s %s, %s, %d\n", \
        cpu->cycleCounter,                                                   \
        cpu->pc,                                                             \
        cpu->virtMem[cpu->pc/4],                                             \
        name,                                                                \
...
#define TRACE_L(cpu, name) do { if (cpu->opts.o_tracePrintEnable) {          \
    printf("[rISA]:[TRACE]:[ %12d cycles ]:  %8x:   %08x   %s %s, %d(%s)\n", \
        cpu->cycleCounter,                                                   \
        cpu->pc,                                                             \
        cpu->virtMem[cpu->pc/4],                                             \
        name,                                                                \
...
#define TRACE_U(cpu, name) do { if (cpu->opts.o_tracePrintEnable) {          \
    printf("[rISA]:[TRACE]:[ %12d cycles ]:  %8x:   %08x   %s %s, %08x\n",   \
        cpu->cycleCounter,                                                   \
        cpu->pc,                                                             \
        cpu->virtMem[cpu->pc/4],                                             \
        name,                                                                \
...
#define TRACE_J(cpu, name) do { if (cpu->opts.o_tracePrintEnable) {          \
    printf("[rISA]:[TRACE]:[ %12d cycles ]:  %8x:   %08x   %s %s, %d\n",     \
        cpu->cycleCounter,                                                   \
        cpu->pc,                                                             \
        cpu->virtMem[cpu->pc/4],                                             \
        name,                                                                \
...
#define TRACE_FEN(cpu, name) do { if (cpu->opts.o_tracePrintEnable) {                     \
    printf("[rISA]:[TRACE]:[ %12d cycles ]:  %8x:   %08x   %s fm:%d, pred:%d, succ:%d\n", \
        cpu->cycleCounter,                                                                \
        cpu->pc,                                                                          \
        cpu->virtMem[cpu->pc/4],                                                          \
        name,                                                                             \
