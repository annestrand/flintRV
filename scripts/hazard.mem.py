...
def genFwd():
    ...
    in_vec  = (
        f"{mem_rd_rw:b}{wb_rd_rw:b}{exec_rs1:05b}{exec_rs2:05b}{mem_rd:05b}{wb_rd:05b}" +
        "00000000000000000000"
    )
    out_vec = f"{rs1_fwd}{rs2_fwd}0000"
...
def genHzd():
    ...
    exec_flush      = bra or jmp or fetch_stall
    in_vec          = (
        "0000000000000000000000" +
        f"{bra:b}{jmp:b}{fetch_valid:b}{mem_valid:b}{exec_mem2reg:b}{fetch_rs1:05b}{fetch_rs2:05b}{exec_rd:05b}"
    )
    out_vec         = f"0000{fetch_stall:b}{exec_stall:b}{exec_flush:b}{mem_flush:b}"
    return in_vec, out_vec

n_vectors = 32

if __name__ == "__main__":
    outfile = f"{basenameNoExt('build', __file__)}_fwd.mem"
    with open(outfile, 'w') as fp:
        outfileGold = f"{basenameNoExt('build', __file__)}_fwd_gold.mem"
    ...
    outfile = f"{basenameNoExt('build', __file__)}_hzd.mem"
    with open(outfile, 'w') as fp:
        outfileGold = f"{basenameNoExt('build', __file__)}_hzd_gold.mem"
        with open(outfileGold, 'w') as fp_gold:
            for i in range(n_vectors):
                # TODO: Add non-random tests too?
